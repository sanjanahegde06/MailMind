...
export async function PATCH(request, context) {
  ...
  try {
    const response = await fetch(
      `${BACKEND_BASE_URL}/tasks/${encodeURIComponent(emailId)}/done?done=${encodeURIComponent(done)}`,
      { method: "PATCH" },
    );
    const data = await response.json().catch(() => ({}));

    if (!response.ok) {
      return Response.json({ error: data?.detail || "Unable to update task" }, { status: response.status });
    }
